...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train a diffusion model on MNIST dataset."
    )
    ...
    parser.add_argument(
        "--VAE_config_file",
        type=str,
        default="./MNIST_VAE.yaml",
        help="Path to the VAE configuration file",
    )
    parser.add_argument(
        "--UNet_config_file",
        type=str,
        default="./MNIST_UNet.yaml",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Number of steps for the noise scheduler",
    )
    parser.add_argument(
        "--betas",
        type=float,
    ...
    parser.add_argument(
        ...
        default="cosine",
        help="Type of noise schedule to use",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of training epochs for diffusion model",
    )
    parser.add_argument(
        "--diffusion_lr",
        type=float,
    ...
    if args.checkpoint_folder is None:
        checkpoint_folder = time.strftime(
            "./checkpoints/%Y-%m-%d_%H-%M-%S", time.localtime()
        )
        os.makedirs(checkpoint_folder, exist_ok=False)
    elif not os.path.exists(args.checkpoint_folder):
        raise ValueError(
            f"Checkpoint folder '{args.checkpoint_folder}' does not exist."
        )
    else:
    ...
    # 2. Instantiate a VAE
    vae = VAE(input_shape=(1, 64, 64), config_file=args.VAE_config_file).to(args.device)
    ...
    print(
        f"{sum(p.numel() for p in vae.parameters() if p.requires_grad)} trainable parameters\n"
    )
    ...
    if os.path.exists(os.path.join(checkpoint_folder, "VAE", "vae.pth")):
        checkpoint = torch.load(
            os.path.join(checkpoint_folder, "VAE", "vae.pth"), map_location=args.device
        )
    ...
    diffusion = Diffusion(
        latent_dim=vae.latent_dim, config_file=args.UNet_config_file
    ).to(args.device)

    print(
        f"Diffusion model successfully instantiated with config file: {args.UNet_config_file}"
    )
    print(
        f"{sum(p.numel() for p in diffusion.parameters() if p.requires_grad)} trainable parameters\n"
    )
