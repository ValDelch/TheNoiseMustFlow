...
class NoiseScheduler(torch.nn.Module):
    ...
    def compute_snr(self, t: TensorOrInt) -> torch.Tensor:
        ...
        if isinstance(t, torch.Tensor):
            assert t.min() >= 0 and t.max() < self.steps, (
                "t must be in the range [0, steps["
            )
        elif isinstance(t, int):
            assert 0 <= t < self.steps, "t must be in the range [0, steps["
        return self.alphas_cumprod[t] / (1.0 - self.alphas_cumprod[t]).clamp(min=1e-8)

    def add_noise_step(
        self, x: torch.Tensor, t: TensorOrInt, noise: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        ...
        if noise is not None:
            assert noise.shape == x.shape, (
                "If noise is provided, it must have the same shape as x"
            )
        else:
    ...
    def add_noise_cumulative(
        ...
        alphas_cumprod = self.alphas_cumprod[
            t
        ]  # (batch_size,) if t is a tensor, else scalar

        # Broadcasting alphas_cumprod if necessary
        if alphas_cumprod.dim() > 0:
            alphas_cumprod = alphas_cumprod.view(
                -1, *(1,) * 3
            )  # Expand alphas_cumprod to match x's dimensions
    ...
    def _geometric_betas(self, beta_start, beta_end) -> torch.Tensor:
        ...
        """
        return beta_start * (beta_end / beta_start) ** (
            torch.arange(self.steps) / (self.steps - 1)
        )
        ...
        if isinstance(t, torch.Tensor):
            ...
            if t.dim() != 0:
                assert t.shape[0] == x.shape[0], (
                    "Batch size of t must match batch size of x"
                )
            elif x.dim() == 4:
                print(
                    "[Warning] t is a single integer but x has batch dimension. Assuming a constant t for all samples."
                )
        elif isinstance(t, int):
            assert 0 <= t < self.steps, "t must be in the range [0, steps["
            if x.dim() == 4:
                print(
                    "[Warning] t is an int but x has batch dimension. Assuming a constant t for all samples."
                )
        else:
